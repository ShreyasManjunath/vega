...
This repo may contain `graphify-out/`.

Use it only as a navigation aid:

- `graphify-out/GRAPH_REPORT.md` gives a quick project map.
- `graphify-out/graph.json` may contain inferred relationships.
- Actual source files and tests are the final source of truth.

Before changing security-sensitive code, inspect the real source files directly.
